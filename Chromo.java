...
*******************************************************************************/
	...
	public Chromo(){
		...
		for (int i=0; i<7; i++)
		{
			...
			{
				if(i < 2)
					chromolist.add("00" + Integer.toBinaryString(i));
				else if (i < 4)
					chromolist.add("0" + Integer.toBinaryString(i));
				else
					chromolist.add(Integer.toBinaryString(i));	
			}	
		}

		Collections.shuffle(chromolist);
		while(!chromolist.isEmpty())
			chromo = chromo + chromolist.remove(0);
	...
	public void doMutation(){
		...
		case 2: 	// Mutation with new representation.
			...
			if(Search.r.nextDouble() < Parameters.mutationRate)
			{
				...
				if(newGene.equals("111")) // Change 111 to 100 which i assume is the bad value for you
					newGene = "000";
				...
				for(int i=0;i<35;i++)
				{
					if(chromolist.get(i).equals(newGene))
					{
						whichInstance--;
						if(whichInstance == 0)
							afterIndex = i;
				...
				chromolist.set(afterIndex, beforeString);

				while(!chromolist.isEmpty())
					mutChromo = mutChromo + chromolist.remove(0);
...
*******************************************************************************/
	...
	public static void mateParents(int pnum1, int pnum2, Chromo parent1, Chromo parent2, Chromo child1, Chromo child2){
		...
		case 2:     //  Two Point Crossover
			...
			for(int i=0; i<35; i++){
				parentlist1.add(Integer.parseInt(parent1.chromo.substring(i*3,i*3+3),2)); // use gray_to_decimal instead of parse :)
				parentlist2.add(Integer.parseInt(parent2.chromo.substring(i*3,i*3+3),2));
			}
			...
			for(int i=0; i<35; i++)
			{
				if(child1list.get(i) < 2)
					child1string = child1string + "00" + Integer.toBinaryString(child1list.get(i));
				else if (child1list.get(i) < 4)
					child1string = child1string + "0" + Integer.toBinaryString(child1list.get(i));
				else
					child1string = child1string + Integer.toBinaryString(child1list.get(i));

				if(child2list.get(i) < 2)
					child2string = child2string + "00" + Integer.toBinaryString(child2list.get(i));
				else if (child2list.get(i) < 4)
					child2string = child2string + "0" + Integer.toBinaryString(child2list.get(i));
				else
					child2string = child2string + Integer.toBinaryString(child2list.get(i));
			}
